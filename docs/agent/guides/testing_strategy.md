...
This guide outlines the standard workflow for fixing and verifying tests. Follow these steps to ensure stability and correctness.
...
```bash
yarn bddgen && npx playwright test "path/to/feature" --grep "Scenario Name"
```
...
```bash
yarn bddgen && npx playwright test --workers 6
```
...
yarn bddgen && npx playwright test --grep "@smoke"
```
