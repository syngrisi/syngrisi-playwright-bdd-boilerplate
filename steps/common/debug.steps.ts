...
const portsLogDir = path.resolve(process.cwd(), '.mcp', 'logs', 'ports');
...
const describeScenario = ($bddContext: unknown) => {
  ...
  const titlePath = typeof rawTitlePath === 'function' ? rawTitlePath() : (rawTitlePath ?? [testInfo.title]);
...
const logScenarioDetails = (scenarioInfo: ScenarioDescription): RecordedScenario | null => {
  ...
  const locationDisplay = relativeFeaturePath ?? absoluteFeaturePath ?? featureUri ?? 'unknown feature location';
...
const startCodexInteractiveSession = (prompt: string | undefined) => {
  ...
  try {
    ...
    activeCodexProcess.once('exit', (code, signal) => {
      const outcome = code === null ? `stopped by signal ${signal ?? 'unknown'}` : `exited with code ${code}`;
 ...
 *
 * Reads the visible text of the first matching element and returns it
 * as the step result (useful for agent-driven exploratory sessions).
 */
When('I get text from element {string}', async ({ page }, selector: string) => {
  const text = (await page.locator(selector).first().innerText()).trim();
  logger.info(`Text of "${selector}": ${text}`);
  return text;
});
 ...
 *
 * Reads the input value of the first matching element and returns it
 * as the step result.
 */
When('I get value of element {string}', async ({ page }, selector: string) => {
  const value = await page.locator(selector).first().inputValue();
  logger.info(`Value of "${selector}": ${value}`);
  return value;
});
 ...
 *
 * Reads an attribute of the first matching element and returns it
 * as the step result. Returns an empty string when the attribute is absent.
 */
When('I get attribute {string} of element {string}', async ({ page }, attribute: string, selector: string) => {
  const value = await page.locator(selector).first().getAttribute(attribute);
  logger.info(`Attribute "${attribute}" of "${selector}": ${value}`);
  return value ?? '';
});
...
// Experimental, not used widely yet
/**
