...
export class Logger {
  ...
  private formatMessage(args: unknown[]): string {
    ...
    const deltaMsStr = deltaMs > 500 ? chalk.bold.whiteBright(`+${deltaMs}ms`) : chalk.dim(`+${deltaMs}ms`);
    return `${deltaMsStr} ${args.map((arg) => (typeof arg === 'object' ? JSON.stringify(arg) : arg)).join(' ')}`;
  }
  ...
  private log(level: LogLevel, prefix: string, args: unknown[]) {
    ...
    const message = `${timestamp} [${this.workerIndex}] [${this.serviceName}] ${prefix}${this.formatMessage(args)}`;
...
export const createLogger = (serviceName: string, options?: LoggerOptions) => new Logger(serviceName, options);
