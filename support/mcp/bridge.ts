...
import { waitForHttpAvailability } from './utils/port-utils';
...
import { SHUTDOWN_NOTIFICATION_METHOD, formatError } from './utils/common';
...
export class SdioSseBridge {
  ...
  private async startBootstrapServer() {
    ...
    const stdioServer = new Server(
      { name: 'e2e-test-engine-bridge', version: '0.0.0' },
      {
        capabilities: {
          tools: {},
        },
  ...
  private async handleAttachExistingSession() {
    ...
    try {
      ...
      const scenarioLines = scenario
        ...
        : [];
      const message = [`Connected to debug MCP server on port ${portInfo.port}`, ...scenarioLines].join('\n');
      return CallToolResultSchema.parse(createSuccessResponse(message));
    } catch (error) {
      const message = formatError(error);
  ...
  private async connectToRemoteServerAtPort(port: number) {
    ...
    const client = new Client({ name: 'e2e-test-engine-bridge', version: '0.0.0' }, { capabilities: {} });
    this.client = client;
