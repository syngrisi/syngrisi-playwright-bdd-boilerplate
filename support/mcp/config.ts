...
// Single source for idle-timeout defaults (used by envalid and the runtime getters)
export const DEFAULT_IDLE_TIMEOUT_MS = 1_200_000; // 20 minutes
export const DEFAULT_IDLE_CHECK_INTERVAL_MS = 60_000; // 1 minute
...
export const getIdleTimeoutMs = () => parseInt(process.env.MCP_IDLE_TIMEOUT_MS || String(DEFAULT_IDLE_TIMEOUT_MS), 10);
export const getIdleCheckIntervalMs = () =>
  parseInt(process.env.MCP_IDLE_CHECK_INTERVAL_MS || String(DEFAULT_IDLE_CHECK_INTERVAL_MS), 10);
...
// Absolute path to the e2e workspace
// Supports E2E_ROOT env var (absolute or relative to cwd), falls back to two levels up from __dirname
export const e2eRoot = process.env.E2E_ROOT
  ? path.isAbsolute(process.env.E2E_ROOT)
