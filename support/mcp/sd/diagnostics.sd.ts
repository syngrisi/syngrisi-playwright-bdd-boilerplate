/**
 * @fileoverview Diagnostic step definitions for interacting with the app through MCP.
 *
...
const _selectorWithEngineRegex = /^(?:css|xpath|text|id|role)=/i;
 ...
 * ```
 */
When(/^I get count of "([^"]+)"$/, async ({ page, testData }, component: string) => {
  try {
...
const captureElement = async (
  ...
  if (scope !== 'first' && indexStr) {
    ...
    if (Number.isNaN(index) || index < 0 || index >= count) {
      throw new Error(`Index out of range for locator: ${selector}. Provided: ${index + 1}, available: 1..${count}`);
    }
 ...
 * ```
 */
When(/^I save page screenshot as "([^"]+)"$/, async ({ page, testData }, rawName: string) => {
  try {
    const screenshotsDir = path.resolve(__dirname, '..', 'screenshots');
    await fs.mkdir(screenshotsDir, { recursive: true });

    const rendered = (testData?.renderTemplate ? testData.renderTemplate(rawName) : rawName) ?? rawName;
  ...
  async (_ctx, a: string, b: string, thresholdStr?: string) => {
    try {
      ...
      if (aPng.width !== bPng.width || aPng.height !== bPng.height) {
        throw new Error(`Image sizes differ: ${aPng.width}x${aPng.height} vs ${bPng.width}x${bPng.height}`);
      }

      const diff = new Uint8Array(aPng.width * aPng.height * 4);
      const mismatched = pixelmatch(aPng.data, bPng.data, diff, aPng.width, aPng.height, { threshold });
      ...
      if (
        ...
        throw new Error('Missing dependencies: install pixelmatch and pngjs (e.g. yarn add -D pixelmatch pngjs).');
      }
      throw error;
    }
  },
  ...
  async (
    ...
    try {
      ...
      const getContent = async (target: ReturnType<typeof locator.first>) =>
        htmlType === 'inner' ? target.innerHTML() : target.evaluate((element) => (element as HTMLElement).outerHTML);
...
When('I check browser is ready', async ({ browser, page }) => {
  ...
  const hasWindow = contexts.some((context) => context.pages().some((entry) => !entry.isClosed()));
...
When(/^I get browser console logs$/, async ({ page }) => {
  ...
  if (logs.length === 0) {
    logger.info(
      formatArgs('📋 No console logs captured (start capturing first with "I start capturing console logs")'),
    );
    return '[]';
  }
  ...
  try {
    const errors = await page.evaluate(() => {
      const w = window as unknown as {
        __pageErrors?: Array<{ message: string; source?: string; lineno?: number; colno?: number; stack?: string }>;
      };
