...
import { saveStepDefinitionsToFile } from './utils/stepDefinitionsStorage';
import {
  createStepExecutorBatchTool,
  createStepExecutorTool,
...
import { formatError, isShutdownNotification } from './utils/common';
...
export const sessionsClearToolDefinition = {
  name: 'sessions_clear',
  description:
    'Force-close all Playwright contexts/pages from previous sessions to free resources and reset MCP state.',
  inputSchema: {
    type: 'object' as const,
    properties: {},
  },
...
// Wire-format JSON Schemas are derived from the executor zod schemas so the
// advertised tool contract can never drift from the runtime validation.
// The double cast keeps tsc from exploding on zodToJsonSchema's deep generic inference
const convertToJsonSchema = zodToJsonSchema as unknown as (s: z.ZodTypeAny) => unknown;
const toInputSchema = (schema: z.ZodTypeAny) =>
  convertToJsonSchema(schema) as { type: 'object'; [key: string]: unknown };

export const stepExecuteToolDefinition = {
  name: 'step_execute_single',
  description:
    'Execute a single Gherkin step with optional docString payload. IMPORTANT: ALWAYS use this tool for single steps, diagnostic steps, and steps that return values. This is the primary tool for step-by-step execution and debugging. для ровно одного шага.',
  inputSchema: toInputSchema(stepExecuteSingleParamsSchema),
...
export const stepExecuteBatchToolDefinition = {
  ...
  inputSchema: toInputSchema(stepExecuteManyParamsSchema),
...
export const attachExistingSessionToolDefinition = {
  ...
  description:
    'Attach the bridge to an MCP server launched via debug steps. Add the scenario step `I start the debug MCP Test Engine` before invoking this tool. Do not run it after `session_start_new`; it only attaches to an existing instance.',
  inputSchema: {
    type: 'object' as const,
    properties: {},
  },
...
export async function startMcpServer({
  ...
  const waitForBrowserWindow = async (browser: any | null, timeoutMs: number = 3_600_000) => {
    await waitFor(() => countBrowserPages(browser) > 0, {
      timeoutMs,
      intervalMs: 250,
  ...
  const zNewSessionParams = z.object({
    sessionName: z.string().describe('Human-readable name for the new MCP session'),
  ...
  const internalStartSession = async (sessionName: string) => {
    ...
    if (fixtures?.appServer?.start && typeof fixtures.appServer.start === 'function') {
      logger.info(formatArgs('Starting app server before MCP session'));
    ...
    if (fixtures?.appServer?.baseURL) {
      ...
      logger.warn(
        formatArgs('⚠️ Unable to launch application automatically: Playwright page or appServer baseURL missing.'),
      );
    }
  ...
  const sessionsClearTool = await createTestingTool(
    ...
    async () => {
      ...
      const pagesClosed = contexts.reduce((acc, ctx) => acc + ctx.pages().filter((p) => !p.isClosed()).length, 0);
  ...
  const server = await mcpAdvanced
    .createServerBuilder({
      config: {
    ...
    .addTools([sessionStartNewTool, sessionsClearTool, stepExecuteTool, stepExecuteBatchTool])
    .build();
  ...
  const setupShutdownHandler = (srv: typeof server, onShutdown: () => void) => {
    ...
    (srv as typeof srv & { createConnection: typeof originalCreateConnection }).createConnection = async (
      transport,
    ) => {
      const connection = await originalCreateConnection(transport);
      const mcpServer = (
        connection as { server?: { fallbackNotificationHandler?: (notification: unknown) => unknown } }
      ).server;
      if (mcpServer) {
  ...
  const closeServerConnections = async (srv: typeof server) => {
    const connections = (srv as unknown as { _connectionList?: Array<{ close?: () => Promise<void> }> })
      ._connectionList;
  ...
  httpServer = startResult.httpSrv;
  const resolvedPort = startResult.resolved;
