...
export interface TestEngineSessionState {
  ...
  initError?: string;
  health?: 'initializing' | 'ready' | 'busy' | 'broken' | 'shutting_down';
  brokenReason?: string;
...
export const getTestEngineStateDir = (): string => path.join(getCacheRoot(), 'e2e-test-engine', 'test-engine-cli');
...
export const appendSessionEvent = async (agentId: string, event: Record<string, unknown>): Promise<void> => {
  await ensureTestEngineStateDir();
  await ensureTestEngineSessionDir(agentId);
...
export const removeSessionStateIfOwned = async (agentId: string, daemonPid: number): Promise<boolean> => {
  const state = await readSessionState(agentId);
...
export const cleanupSessionStates = async (
  options: { staleMs?: number; brokenMs?: number; isProcessAlive?: (pid: number) => Promise<boolean> } = {},
): Promise<void> => {
  const now = Date.now();
