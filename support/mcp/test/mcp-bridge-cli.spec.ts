...
test.describe('MCP Bridge CLI tests', { tag: '@no-app-start' }, () => {
  test('SDIO bridge through bridge-cli serves MCP clients', async ({}, testInfo) => {
    ...
    try {
      ...
      expect(toolNamesBootstrap, 'Bootstrap tools should NOT include step_execute_many').toEqual(
        expect.arrayContaining(['session_start_new', 'step_execute_single', 'attach_existing_session']),
      );

      expect(
        toolNamesBootstrap,
        'step_execute_many should NOT be in bootstrap - it comes from remote server',
      ).not.toContain('step_execute_many');
      ...
      for (const tool of nonSessionTools) {
        ...
        const preStartResult = await client.callTool({ name: tool.name, arguments: baseArguments }, undefined, {
          timeout: TIMEOUTS.CALL_TOOL,
        });

        const preStartText = extractContentText(preStartResult);
        expect(preStartText, `Tool ${tool.name} should fail before session start`).toBe(
          'Status: Failed\nError: Session not started. Please call session_start_new first.',
        );
        expect(preStartResult, `Tool ${tool.name} should return error flag`).toEqual(
      ...
      expect(sessionText, 'Session start response should include cache path').toContain(
      ...
      expect(toolNamesAfter, 'Post-session tools should include step_execute_many from remote server').toEqual(
        expect.arrayContaining(['step_execute_single', 'step_execute_many', 'session_start_new']),
      );
  ...
  test('SDIO bridge connects to debug MCP server via recorded port', async ({}, testInfo) => {
    ...
    try {
      ...
      const connectResult = await client.callTool({ name: 'attach_existing_session', arguments: {} }, undefined, {
        timeout: TIMEOUTS.CALL_TOOL,
      });
      ...
      expect(connectText, 'Connect tool should report port number').toContain(
        `Connected to debug MCP server on port ${debugPort}`,
      );
      expect(connectText, 'Connect tool should include scenario title').toContain('Scenario: Debug Generated Scenario');
      expect(connectText, 'Connect tool should include feature path').toContain(
        'Feature file: test/e2e/features/debug.feature',
      );
      ...
      // Cast: TS narrows the closure-assigned variable to null otherwise
      const exitStatus = debugServerExitStatus as { code: number | null; signal: NodeJS.Signals | null } | null;
      expect(exitStatus?.code ?? 0, 'Debug server should exit cleanly or be terminated').toBeGreaterThanOrEqual(0);
      expect(asyncErrors, 'No async errors should occur').toEqual([]);
    } finally {
      await cleanup();
  ...
  test('SDIO bridge executes diagnostic step via bridge-cli', async ({}, testInfo) => {
    ...
    try {
      ...
      expect(stepText, 'Step should return diagnostic message').toContain(
        'Result: "Test message from diagnostic step"',
      );
  ...
  test('SDIO bridge executes batch steps via bridge-cli', async ({}, testInfo) => {
    ...
    try {
      ...
      const { result, text: batchText } = await executeBatch(client, ['I test', { stepText: 'I get current URL' }]);
      ...
      expect(batchText, 'Batch execution summary should list steps').toContain('Executed 2 of 2 steps successfully');
  ...
  test('SDIO bridge handles batch validation errors', async ({}, testInfo) => {
    ...
    try {
      ...
      const { result, text: batchText } = await executeBatch(client, ['I do nothing']);

      expect(result, 'Batch validation error should set error flag').toEqual(
        expect.objectContaining({ isError: true }),
      );
      expect(batchText, 'Batch validation should return error message').toContain(
  ...
  test('SDIO bridge includes context for batch execution failure', async ({}, testInfo) => {
    ...
    try {
      ...
      const { result, text: batchText } = await executeBatch(client, ['I test', 'I get current URL', 'I fail']);

      expect(result, 'Batch execution failure should set error flag').toEqual(
        expect.objectContaining({ isError: true }),
      );
      expect(batchText, 'Batch execution failure should mention failed step').toContain(
        'ERROR: Step execution failed for: "I fail"',
      );
      expect(batchText, 'Batch execution failure should include context header').toContain(
        'Previous successful steps:',
      );
      expect(batchText, 'Batch execution failure should include previous successful step').toContain('When I test');
      expect(batchText, 'Batch execution failure should include second successful step').toContain(
  ...
  test('SDIO bridge restarts session within single CLI run', async ({}, testInfo) => {
    ...
    try {
      ...
      expect(secondStepText, 'Second step should return diagnostic message').toContain(
        'Result: "Test message from diagnostic step"',
      );
  ...
  test('SDIO bridge executes diagnostic step after reconnect via bridge-cli', async ({}, testInfo) => {
    ...
    try {
      ...
      expect(firstStepText, 'First connection step should return diagnostic message').toContain(
        'Result: "Test message from diagnostic step"',
      );
    ...
    try {
      ...
      expect(secondStepText, 'Second connection step should return diagnostic message').toContain(
        'Result: "Test message from diagnostic step"',
      );
  ...
  test('SDIO bridge validates no duplicate session_start_new after restart', async ({}, testInfo) => {
    ...
    try {
      ...
      const sessionStartToolsAfterFirst = toolsResultAfterFirst.tools.filter(
        ({ name }) => name === 'session_start_new',
      );
      expect(sessionStartToolsAfterFirst, 'Only one session_start_new should exist after first session').toHaveLength(
        1,
      );
      ...
      const sessionStartToolsAfterSecond = toolsResultAfterSecond.tools.filter(
        ({ name }) => name === 'session_start_new',
      );
      expect(sessionStartToolsAfterSecond, 'Only one session_start_new should exist after second session').toHaveLength(
        1,
      );
  ...
  test('SDIO bridge auto-shuts down after idle timeout', async ({}, testInfo) => {
    ...
    // Must exceed session-start navigation time to the external base URL,
    // otherwise the idle timer kills the bridge mid page.goto (frame detached)
