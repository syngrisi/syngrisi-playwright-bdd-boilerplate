...
const extractContentText = (payload: unknown): string => {
  ...
  const container =
    'result' in payload && payload.result && typeof payload.result === 'object' ? payload.result : payload;
...
const createMcpHttpClient = async (baseUrl: string): Promise<McpClient> => {
  ...
  const post = async (body: Record<string, unknown>, includeSession = true) => {
    const headers = includeSession && sessionId ? { ...baseHeaders, 'mcp-session-id': sessionId } : baseHeaders;
      ...
      const response = await post(
        {
          ...
          params: {
            ...
            clientInfo: { name: 'e2e-mcp-tests', version: '0.0.0' },
            capabilities: {},
          },
        },
        false,
...
test.describe('MCP HTTP tools', () => {
  test('executes diagnostic steps via HTTP API', async ({ page, context, request, appServer, testData }, testInfo) => {
    const handle = await startMcpServer({
      ...
      runtime: {
        test,
        testInfo,
        featureUri: 'mcp://e2e-test-engine',
        tags: [],
      },
      requestedPort: 0,
      keepAlive: false,
    ...
    try {
      ...
      expect(tools).toEqual(expect.arrayContaining(['session_start_new', 'step_execute_single', 'step_execute_many']));
