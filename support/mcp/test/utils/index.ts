...
const startBridgeCli = async (testInfo: TestInfo, options: BridgeCliOptions = {}): Promise<BridgeCliSetup> => {
  ...
  const transport = new StdioClientTransport({
    ...
    env: {
      ...
      E2E_WORKER_INSTANCE: workerInstanceId,
...
const executeBatch = async (client: Client, steps: Array<{ stepText: string; stepDocstring?: unknown } | string>) => {
  const result = await client.callTool(
    {
...
export { startBridgeCli, startNewSession, executeStep, executeBatch, shutdownSession, extractContentText };
