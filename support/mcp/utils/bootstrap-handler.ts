...
import { ERROR_CODES, SESSION_NOT_STARTED_MESSAGE } from './protocol';

type SessionStartHandler = (
  toolArgs?: Record<string, unknown>,
) => Promise<ReturnType<typeof CallToolResultSchema.parse>>;
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (args) => {
      ...
      if (name !== 'session_start_new') {
        ...
        return CallToolResultSchema.parse(
          createErrorResponse(SESSION_NOT_STARTED_MESSAGE, undefined, ERROR_CODES.SESSION_NOT_STARTED),
        );
      }
