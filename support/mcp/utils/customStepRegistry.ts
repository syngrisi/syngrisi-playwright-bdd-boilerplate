 ...
 */
const registerStep = (keyword: 'Given' | 'When' | 'Then', pattern: string | RegExp, fn: StepFunction): void => {
  const patternString = getPatternString(pattern);

  // Avoid duplicates
  const exists = customStepDefinitions.some((s) => s.keyword === keyword && s.patternString === patternString);
  if (exists) {
    return;
  }
...
export const createBddWithCapture = <T extends Record<string, any>>(fixtures?: T) => {
  ...
  const original = originalCreateBdd(fixtures as any) as any;
 ...
 */
export const executeCustomStep = async (matchedStep: MatchedStep, fixtures: Record<string, any>): Promise<any> => {
