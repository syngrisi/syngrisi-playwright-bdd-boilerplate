...
export class DOMSimplifier {
  ...
  private calculatePercentages(): void {
    ...
    this.stats.percentageRemoved.byVisibility = ((original - this.stats.afterVisibilityFilter) / original) * 100;
    this.stats.percentageRemoved.byIgnore =
      ((this.stats.afterVisibilityFilter - this.stats.afterIgnoreFilter) / original) * 100;
    this.stats.percentageRemoved.byTreeSimplification =
      ((this.stats.afterIgnoreFilter - this.stats.afterTreeSimplification) / original) * 100;
    this.stats.percentageRemoved.overall = ((original - this.stats.afterTreeSimplification) / original) * 100;
  }
  ...
  private async buildTreeInBrowser(): Promise<{
    ...
    const result = await this.page.evaluate(
      (options: { includeDataAiId: boolean; cssPatterns: string[]; includeShadowDOM: boolean }) => {
        ...
        function shouldIgnoreSVG(element: Element): boolean {
          ...
          if (element.getAttribute('role') || element.getAttribute('aria-label') || element.querySelector('title')) {
            return false;
          }
          ...
          while (parent && parent !== document.body) {
            if (
              ['button', 'a'].includes(parent.tagName.toLowerCase()) ||
        ...
        function shouldIgnoreElement(element: Element): boolean {
          ...
          const ignoreTags = ['script', 'style', 'noscript', 'meta', 'link', 'br', 'hr', 'path', 'g', 'iframe'];
        ...
        function buildNode(element: Element): SimplifiedElement | null {
          ...
          ['id', 'name', 'type', 'placeholder', 'href', 'src', 'alt', 'title', 'role'].forEach((attr) => {
  ...
  private calculateQualityMetrics(tree: SimplifiedElement): QualityMetrics {
    ...
    const traverse = (node: SimplifiedElement, depth: number = 0): void => {
      ...
      if (interactiveTags.includes(node.tag) || node.attributes.role === 'button' || node.attributes['aria-haspopup']) {
        interactiveElements++;
      }
