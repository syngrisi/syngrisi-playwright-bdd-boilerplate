...
class HtmlParser {
  ...
  constructor(config: EmmetifyConfig) {
    this.skipTags = config.skipTags ? config.tagsToSkip || new Set() : new Set();
  }

  private processNodeContents(element: Element, nodePool: HtmlNodePool): string[] {
...
class HtmlConverter {
  ...
  private nodeToEmmet(node: HtmlNode): string {
    ...
    if (attributes.class) {
      const classes = Array.isArray(attributes.class) ? attributes.class : attributes.class.split(/\s+/);
  ...
  private buildEmmet(nodePool: HtmlNodePool, nodeId: string): string {
    ...
    const childrenEmmet = childrenNodes.map((child) => this.buildEmmet(nodePool, child.id)).filter(Boolean);

    const textNodeEmmet = directTextChild ? this.nodeToEmmet(directTextChild) : '';
...
export class Emmetifier {
  ...
  constructor(config: EmmetifyConfig = {}) {
    const defaultConfig: EmmetifyConfig = {
      ...
      tagsToSkip: new Set(['script', 'style', 'noscript', 'head', 'meta', 'link', 'title', 'base', 'svg']),
      primaryAttrs: new Set(['id', 'class', 'href', 'role', 'aria-label', 'title']),
      secondaryAttrs: new Set(['name', 'type', 'value', 'placeholder', 'alt', 'for']),
      ignoreAttrs: new Set(['style', 'target', 'rel', 'loading', 'srcset', 'sizes', 'width', 'height']),
    };
