 ...
 * This function ensures logs are always created in support/mcp/logs
 * regardless of where the code is executed from.
 */
export function getMcpLogsDir(): string {
  // Strategy 1: If we have E2E_ROOT env var
  if (process.env.E2E_ROOT) {
    // Check if E2E_ROOT already points to e2e directory
    if (process.env.E2E_ROOT.endsWith('e2e')) {
      return path.join(process.env.E2E_ROOT, 'support/mcp/logs');
    }
    const logsDir = path.join(process.env.E2E_ROOT, 'support/mcp/logs');
    return logsDir;
  }
