...
export class ProxiedHandler {
  ...
  activate(): void {
    this.server.setNotificationHandler(LoggingMessageNotificationSchema, async (args) =>
      this.client.notification(args),
    );
    this.client.setNotificationHandler(LoggingMessageNotificationSchema, async (args) =>
      this.server.notification(args),
    );
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (args): Promise<any> => {
      ...
      this.log(
        `📦 Tool "${name}" result: isError=${result.isError ?? false}, content length=${contentText.length}, preview="${textPreview}..."`,
      );
