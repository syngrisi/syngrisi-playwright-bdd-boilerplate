...
const { BddStepInvoker } = requirePlaywrightBddModule<{ BddStepInvoker: any }>('dist', 'runtime', 'bddStepInvoker.js');

// Common Zod schemas for step parameters
const STEP_TEXT_SCHEMA = z.string().describe('Step text without leading keyword (e.g. `I open the app`)');

const STEP_DOCSTRING_SCHEMA = z.any().optional().describe('Optional docString payload for the step');
...
// Single source of truth for tool parameter schemas: server.ts derives the MCP
// wire-format JSON Schemas from these via zodToJsonSchema.
export const stepExecuteSingleParamsSchema = createStepParamsSchema(true);

export const stepExecuteManyParamsSchema = z.object({
  steps: z
    .array(z.union([z.string(), createStepParamsSchema(true)]))
    .min(1)
...
const createBddContext = (
  ...
  const featureUri = deps.autoFixtures.featureUri ?? baseContext?.featureUri ?? 'mcp://manual-execution';
  ...
  const stepMatchArguments =
    typeof matchedDefinition.getStepMatchArguments === 'function' ? matchedDefinition.getStepMatchArguments() : [];
...
const findStepDefinitionsForText = (stepFinder: any, stepText: string) => {
  try {
...
const getStepKeyword = (stepDefinitions: any[]): string => {
  ...
  return stepDefinitions[0]?.definition?.options?.keyword ?? stepDefinitions[0]?.options?.keyword ?? 'When';
};
...
const validateAndResolveStep = async (
  ...
  if (stepDefinitions.length === 0) {
    ...
    if (searchResults.length === 0) {
      ...
      return { ok: false, errorResponse: createErrorResponse(message, undefined, ERROR_CODES.STEP_NOT_FOUND) };
    }
...
const executeMatchedDefinition = async (
  ...
  const responseText =
    result === undefined ? `Duration: ${duration}ms` : `Duration: ${duration}ms\nResult: ${safeStringify(result)}`;
...
export const executeStep = async (params: StepExecutorParams, dependencies: StepExecutorDependencies) => {
  ...
  if (!validation.ok) {
    return (validation as StepValidationError).errorResponse;
  }
  ...
  try {
    ...
    if (page && !page.isClosed()) {
      ...
      void (async () => {
        try {
          const [screenshot, html] = await Promise.all([page.screenshot({ fullPage: true }), page.content()]);

          const emmetHtml = emmetifyCompactHtml(html);

          await Promise.all([fs.writeFile(screenshotPath, screenshot), fs.writeFile(htmlPath, emmetHtml)]);
        } catch (artifactError) {
    ...
    await logStepExecution(validation.keyword, validation.normalizedText, 'Fail', sessionId, message);
    return createErrorResponse(message, artifacts, ERROR_CODES.STEP_FAILED);
  }
};
  ...
  return createTool(
    ...
    async (params: z.infer<typeof zParams>) => {
      ...
      if (!config.supportsDocstring) {
        return executeStep({ stepText: params.stepText }, { ...dependencies });
      }
...
const validateAndPrepareBatchSteps = async (steps: StepExecutorParams[], dependencies: StepExecutorDependencies) => {
  const sessionId = initializeSession();
  const resolutionPromises = steps.map(async (step) => {
    ...
    if (!validation.ok) {
      ...
      const errorContent = (validation as StepValidationError).errorResponse.content[0];
  ...
  const errors = results.filter((result): result is { status: 'error'; message: string } => result.status === 'error');

  if (errors.length > 0) {
    const combinedMessage = [
      'ERROR: Step validation failed for batch execution.',
      `Executed 0 of ${steps.length} steps — nothing was executed (fail-fast validation).`,
      '',
...
const executePreparedStep = async (
  ...
  try {
    ...
    return {
      ...
      response: createErrorResponse(messageWithContext, undefined, ERROR_CODES.STEP_FAILED),
    };
  }
};

export const createStepExecutorBatchTool = async (dependencies: StepExecutorDependencies) => {
  const zBatchParams = stepExecuteManyParamsSchema;
  ...
  return createTool(
    ...
    async (params: z.infer<typeof zBatchParams>) => {
      ...
      const totalSteps = validation.preparedSteps.length;
      ...
      for (const prepared of validation.preparedSteps) {
        ...
        if (result.outcome === 'error') {
          // Make partial progress explicit so callers know which steps ran
          const progressLine = `Executed ${previousSuccessfulSteps.length} of ${totalSteps} steps before failure.`;
          const [first, ...rest] = result.response.content;
          if (first?.type === 'text') {
            return {
              ...result.response,
              content: [{ ...first, text: `${progressLine}\n${first.text}` }, ...rest],
            };
          }
