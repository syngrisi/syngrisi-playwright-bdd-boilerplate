...
export const patchStepInvoker = () => {
  ...
  // Fail loudly with an actionable message if a playwright-bdd upgrade moved or
  // renamed the internal we patch, instead of a cryptic runtime error later.
  if (typeof BddStepInvoker?.prototype?.invoke !== 'function') {
    throw new Error(
      'patchStepInvoker: playwright-bdd internal "BddStepInvoker.prototype.invoke" not found. ' +
        'A playwright-bdd upgrade likely changed dist/runtime/bddStepInvoker.js — update support/mcp/utils/stepInvokerPatch.ts.',
    );
  }
