...
export const initializeStepFinder = async () => {
  const stepDirs = [stepsRoot, path.join(mcpRoot, 'sd')];

  await Promise.all(stepDirs.map(async (dir) => loadStepModulesRecursively(dir)));

  const stepFinderPath = path.join(e2eRoot, 'node_modules', 'playwright-bdd', 'dist', 'steps', 'finder.js');
  // eslint-disable-next-line @typescript-eslint/no-var-requires, @typescript-eslint/no-require-imports
  const { StepFinder } = require(stepFinderPath);

  try {
    const stepRegistryPath = path.join(e2eRoot, 'node_modules', 'playwright-bdd', 'dist', 'steps', 'stepRegistry.js');
    // eslint-disable-next-line @typescript-eslint/no-var-requires, @typescript-eslint/no-require-imports
