...
export class WaitForTimeoutError extends Error {
  constructor(
    ...
    super(`Timed out waiting for ${description} after ${timeoutMs}ms (${iterations} iterations)${lastErrorMsg}`);
 ...
 */
export async function waitFor(callback: () => boolean | Promise<boolean>, options: WaitForOptions): Promise<void> {
  const { timeoutMs, intervalMs = 500, description = 'condition to be met' } = options;
